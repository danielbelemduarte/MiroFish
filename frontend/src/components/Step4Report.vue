    ...
    <div class="main-split-layout">
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">A gerar {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - Mostrar após conclusão -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (conteúdo gerado, mas a secção completa pode ainda não estar pronta) -->
                  ...
                  <!-- Section Complete (secção gerada com sucesso) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Quando é a resposta final, mostrar indicação especial -->
...
const toggleRawResult = (timestamp, event) => {
  // Guardar a posição do botão relativamente ao viewport
...
const toggleSectionCollapse = (idx) => {
  // Apenas secções concluídas podem ser recolhidas
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  ...
  'panorama_search': {
    ...
    color: 'blue',
    icon: 'globe' // Ícone de globo - representa pesquisa panorâmica
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Ícone de utilizadores - representa diálogo
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Ícone de relâmpago - representa rapidez
  },
  ...
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Ícone de base de dados - representa entidades
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/Questão de análise:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extrair cenário de previsão
    const reqMatch = text.match(/Cenário de previsão:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Extrair dados estatísticos - corresponde ao formato "Factos de previsão relacionados: X"
    const factMatch = text.match(/Factos de previsão relacionados:\s*(\d+)/)
    const entityMatch = text.match(/Entidades envolvidas:\s*(\d+)/)
    const relMatch = text.match(/Cadeias de relações:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extrair sub-questões - extração completa, sem limite de quantidade
    const subQSection = text.match(/### Sub-questões analisadas\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extrair factos-chave - extração completa, sem limite de quantidade
    const factsSection = text.match(/### 【Factos-chave】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Extrair entidades principais - extração completa, incluindo resumo e contagem de factos relacionados
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/Factos relacionados:\s*(\d+)/)
        return {
    ...
    // Extrair cadeias de relações - extração completa, sem limite de quantidade
    const relSection = text.match(/### 【Cadeias de relações】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const nodesMatch = text.match(/Total de nós:\s*(\d+)/)
    const edgesMatch = text.match(/Total de arestas:\s*(\d+)/)
    const activeMatch = text.match(/Factos ativos atuais:\s*(\d+)/)
    const histMatch = text.match(/Factos históricos\/expirados:\s*(\d+)/)
    ...
    // Extrair factos ativos atuais - extração completa, sem limite de quantidade
    const activeSection = text.match(/### 【Factos ativos atuais】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Extrair factos históricos/expirados - extração completa, sem limite de quantidade
    const histSection = text.match(/### 【Factos históricos\/expirados】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Entrevistados:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extrair razão de seleção dos entrevistados
    const reasonMatch = text.match(/### Razões da seleção dos entrevistados\n([\s\S]*?)(?=\n---\n|\n### Registo das entrevistas)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Exemplo: 1. **校友_345(index=1)**: como ex-aluno...
        ...
        // Exemplo: - selecionar 家长_601(index 0): como representante do grupo de pais...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Selecionar\s+([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // Exemplo: - **家长_601(index 0)**: como representante do grupo de pais...
        if (!headerMatch) {
        ...
        if (name) {
          // Guardar a razão da pessoa anterior
        ...
        } else if (currentName && line.trim() && !line.match(/^Não selecionado|^Em resumo|^Seleção final/)) {
          // Continuação da razão (excluir parágrafo de resumo final)
          currentReason.push(line.trim())
        }
      }
      
      // Guardar a razão da última pessoa
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Extrair cada registo de entrevista
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extrair título (ex: "estudante", "profissional de educação", etc.)
      const titleMatch = block.match(/^(.+?)\n/)
      if (titleMatch) interview.title = titleMatch[1].trim()
      
      // Extrair nome e função
      ...
      // Extrair biografia
      const bioMatch = block.match(/_Biografia:\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Se a primeira pergunta tem "1." antes, necessita tratamento especial
      ...
      if (answerMatch) {
        ...
        // Separar respostas do Twitter e Reddit
        const twitterMatch = answerText.match(/【Resposta na plataforma Twitter】\n?([\s\S]*?)(?=【Resposta na plataforma Reddit】|$)/)
        const redditMatch = answerText.match(/【Resposta na plataforma Reddit】\n?([\s\S]*?)$/)
        ...
        // Lógica de fallback de plataforma (compatibilidade com formato antigo: apenas uma marcação de plataforma)
        if (!twitterMatch && redditMatch) {
          // Apenas resposta do Reddit, copiar como exibição padrão apenas quando não é texto de placeholder
          ...
          // Sem marcação de plataforma (formato muito antigo), usar como resposta completa
      ...
      if (quotesMatch) {
        ...
        // Corresponder prioritariamente ao formato > "text"
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/Consulta de pesquisa:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extrair quantidade de resultados
    const countMatch = text.match(/Encontrados\s*(\d+)/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extrair factos relacionados - extração completa, sem limite de quantidade
    const factsSection = text.match(/### Factos relacionados:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Tentar extrair informação de arestas (se existir)
    ...
    // Tentar extrair informação de nós (se existir)
    const nodesSection = text.match(/### Nós relacionados:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Cadeias de relações (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Sub-questões (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Factos-chave mais recentes na memória temporal'),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Cadeias de relações'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questões geradas por consulta de desvio'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Sem entidades principais'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Sem cadeias de relações')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memórias ativas atuais (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memórias ativas atuais'),
          ...
          ) : h('div', { class: 'empty-state' }, 'Sem memórias ativas atuais'),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Tentar extrair informação temporal [time - time]
                  (() => {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Obter a seleção de plataforma atual de uma pergunta
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Definir a seleção de plataforma de uma pergunta
    ...
    // Verificar se é texto de placeholder de plataforma
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(Sem resposta nesta plataforma)' || t === '[Sem resposta]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestãoX：" ou "QuestãoX:" (formato português, novo formato do backend)
      // 2. "1. " ou "\n1. " (número+ponto, compatibilidade com formato antigo)
      let matches = []
      let match

      // Tentar prioritariamente o formato "Questão X："
      const cnPattern = /(?:^|[\r\n]+)Questão\s*(\d+)[：:]\s*/g
      ...
      // Se não correspondeu, fazer fallback para o formato "número."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Se não encontrou numeração ou encontrou apenas uma, retornar como um todo
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^Questão\s*\d+[：:]\s*/, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
    ...
    // Obter a resposta correspondente a uma pergunta
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Divisão falhou: primeira pergunta retorna resposta completa, restantes retornam vazio
      return qIdx === 0 ? answer : ''
    }
    
    // Verificar se uma pergunta tem respostas de duas plataformas (filtrar texto de placeholder)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Botões de alternância entre plataformas (mostrar apenas quando há respostas reais de duas plataformas)
                  ...
                  // Expand/Collapse Button (não mostrar para texto de placeholder)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Pesquisa: '),
      ...
      showTabs.value && h('div', { class: 'quicksearch-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Factos (${props.result.facts.length})`)
        ]),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Resultados da pesquisa'),
            h('span', { class: 'panel-count' }, `Total: ${props.result.facts.length}`)
          ]),
...
const activeStep = computed(() => {
  ...
  return steps[0] || { noLabel: '--', title: 'A aguardar início', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Remover título de nível 2 no início (## xxx), pois o título da secção já é exibido externamente
  ...
  // Processar listas - suporta sublistas
  ...
  // Encapsular listas não ordenadas
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Encapsular listas ordenadas
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Limpar todos os espaços em branco entre itens de lista
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Limpar espaços em branco após tags de início de lista
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Limpar espaços em branco antes das tags de fim de lista
  ...
  // Processar linhas de separação
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Processar quebras de linha - linhas vazias tornam-se separadores de parágrafo, quebra simples torna-se <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Limpar situações de <p><br> seguidos de elementos de bloco (causado por linhas vazias extra)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Limpar tags <br> consecutivas
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Limpar <br> antes de tags de início de parágrafo logo após elementos de bloco
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corrigir numeração de listas ordenadas não contíguas: quando <ol> de item único é separado por conteúdo de parágrafo, manter numeração crescente
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - secção gerada com sucesso
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Expandir automaticamente a secção recém-gerada
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Garantir que o estado de loading é limpo
            emit('update-status', 'completed')
            stopPolling()
            // Lógica de scroll processada uniformemente no nextTick após o fim do ciclo
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Se a tarefa estiver concluída, fazer scroll para o topo; caso contrário, fazer scroll para o fundo para seguir os logs mais recentes
            if (isComplete.value) {
...
// Extrair conteúdo da resposta final - extrair conteúdo da secção da resposta LLM
const extractFinalContent = (response) => {
  ...
  // Tentar encontrar conteúdo após Final Answer: (suporta vários formatos)
  ...
  const chineseFinalMatch = response.match(/Resposta final[:：]\s*\n*([\s\S]*)$/i)
  ...
  // Se o conteúdo é longo e contém formato markdown, tentar remover o processo de pensamento e retornar
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Cinzento escuro, não muda com o estado */
  font-weight: 500;
}
...
/* Console Logs - Manter consistente com Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
