          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- DO_NOTHING: Sem operação (silencioso) -->
                ...
                <!-- Fallback genérico: tipo desconhecido ou com content mas não tratado acima -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Número máximo de rodadas passado do Step2
  minutesPerRound: {
    type: Number,
    default: 30 // Predefinição: 30 minutos por rodada
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const actionIds = ref(new Set()) // Conjunto de IDs de ações para desduplicação
const scrollContainer = ref(null)

// Computed
// Mostrar ações em ordem cronológica (mais recentes em último, ou seja, no fundo)
...
// Reiniciar todos os estados (para reiniciar a simulação)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('Erro: simulationId em falta')
    return
  }

  // Reiniciar todos os estados primeiro, para garantir que não são afetados pela simulação anterior
  resetAllState()

  isStarting.value = true
  startError.value = null
  addLog('A iniciar simulação paralela em duas plataformas...')
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Rodadas máximas de simulação definidas: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Registos antigos limpos, simulação reiniciada')
      }
...
// Rastrear a rodada anterior de cada plataforma, para detetar alterações e registar em log
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detetar alterações de rodada em cada plataforma e registar em log
      ...
      // Verificar se a simulação foi concluída (através de runner_status ou estado de conclusão da plataforma)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Verificação adicional: se o backend ainda não atualizou runner_status, mas as plataformas já reportaram conclusão
      // Verificar através dos estados twitter_completed e reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Fim de simulação detetado em todas as plataformas')
        }
    ...
    console.warn('Falha ao obter estado de execução:', err)
  }
}

// Verificar se todas as plataformas ativadas foram concluídas
const checkPlatformsCompleted = (data) => {
  // Se não houver dados de plataforma, retornar false
  if (!data) return false
  
  // Verificar o estado de conclusão de cada plataforma
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Se pelo menos uma plataforma concluiu, verificar se todas as plataformas ativadas concluíram
  // Determinar se a plataforma está ativada através de actions_count (se count > 0 ou running foi true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Se nenhuma plataforma estiver ativada, retornar false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Adicionar novas ações incrementalmente (com desduplicação)
      ...
      // Não fazer scroll automático, permitir que o utilizador navegue livremente na timeline
      // Novas ações são adicionadas no fundo
    }
  } catch (err) {
    console.warn('Falha ao obter estado detalhado:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Pedido de geração de relatório enviado, por favor aguarde...')
    return
  }

  isGeneratingReport.value = true
  addLog('A iniciar geração de relatório...')
...
onMounted(() => {
  addLog('Passo 3 - Inicialização da simulação')
  if (props.simulationId) {
    doStartSimulation()
  }
})
