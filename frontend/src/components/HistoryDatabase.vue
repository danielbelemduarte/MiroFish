<template>
  ...
  >
    <!-- Decoração de fundo: grelha técnica (visível apenas com projetos) -->
    ...
    <!-- Contentor de cartões (visível apenas com projetos) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Cabeçalho do cartão: simulation_id e estado de disponibilidade -->
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Se houver mais ficheiros, mostrar indicação -->
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} files
            </div>
          </div>
          <!-- Placeholder quando sem ficheiros -->
        ...
        <!-- Título do cartão (primeiros 20 caracteres do requisito de simulação) -->
        ...
        <!-- Linha decorativa inferior (expande no hover) -->
      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Linha divisória de reprodução da simulação -->
...
const selectedProject = ref(null)  // Projeto atualmente selecionado (para o modal)
let observer = null
let isAnimating = false  // Bloqueio de animação, evitar cintilação
let expandDebounceTimer = null  // Temporizador de debounce
let pendingState = null  // Registar o estado alvo pendente de execução

// Configuração do layout dos cartões - ajustado para proporção mais larga
const CARDS_PER_ROW = 4
const CARD_WIDTH = 280  
const CARD_HEIGHT = 280 
const CARD_GAP = 24

// Cálculo dinâmico do estilo de altura do contentor
const containerStyle = computed(() => {
  ...
  // Estado expandido: calcular altura dinamicamente com base no número de cartões
  ...
  // Calcular a altura necessária: linhas * altura do cartão + (linhas-1) * espaçamento + margem inferior
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    // Estado expandido: layout em grelha
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'

    const col = index % CARDS_PER_ROW
    const row = Math.floor(index / CARDS_PER_ROW)
    
    // Calcular o número de cartões na linha atual, garantindo centralização
    ...
    const x = offset * 35
    // Ajustar posição inicial, perto do título mas com espaçamento adequado
...
// Obter classe de estilo com base no progresso das rondas
...
// Formatar data (mostrar apenas a parte da data)
...
// Formatar hora (mostrar horas:minutos)
...
// Formatar exibição de rondas (ronda atual/total de rondas)
...
// Obter tipo de ficheiro (para estilos)
...
// Obter texto da etiqueta do tipo de ficheiro
...
// Truncar nome do ficheiro (preservar extensão)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Atualizar o estado alvo pendente (registar sempre o estado mais recente, mesmo durante animação)
        pendingState = shouldExpand
        
        // Limpar temporizador de debounce anterior (nova intenção de scroll substitui a anterior)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // Se estiver a animar, apenas registar o estado, processar após a animação terminar
        ...
        // Usar debounce para atrasar a mudança de estado, evitar cintilação rápida
        // Atraso curto ao expandir (50ms), atraso maior ao recolher (200ms) para maior estabilidade
        const delay = shouldExpand ? 50 : 200
        
        expandDebounceTimer = setTimeout(() => {
          // Verificar se está a animar
          if (isAnimating) return
          
          // Verificar se o estado pendente ainda precisa ser executado (pode ter sido substituído por scroll posterior)
          ...
          // Desbloquear após animação concluída e verificar se há mudanças de estado pendentes
          setTimeout(() => {
            isAnimating = false
            
            // Após a animação terminar, verificar se há novo estado pendente
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Atrasar um pouco antes de executar, evitar mudança demasiado rápida
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // Usar múltiplos limiares para deteção mais suave
      threshold: [0.4, 0.6, 0.8],
      // Ajustar rootMargin, contrair a parte inferior do viewport para cima, requer mais scroll para acionar expansão
...
// Observar mudanças de rota, recarregar dados ao voltar à página inicial
...
onMounted(async () => {
  // Garantir que o DOM foi renderizado antes de carregar dados
  await nextTick()
  await loadHistory()
  
  // Aguardar renderização do DOM antes de inicializar o observador
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
/* Usar padrão de fundo CSS para criar grelha quadrada com espaçamento fixo */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Posicionar a partir do canto superior esquerdo, ao mudar altura expande apenas na parte inferior sem afetar a posição da grelha existente */
...
.cards-container {
  ...
  /* min-height calculado dinamicamente por JS, adaptável ao número de cartões */
}
...
/* Grupo de ícones de estado de funcionalidades */
.card-status-icons {
  display: flex;
  align-items: center;
  gap: 6px;
...
/* Exibição do progresso de rondas */
.card-progress {
  display: flex;
  align-items: center;
  gap: 6px;
...
.card-progress.in-progress { color: #F59E0B; }  /* Em progresso - laranja */
.card-progress.not-started { color: #9CA3AF; }  /* Não iniciado - cinzento */
...
/* Estilo minimalista de etiqueta de ficheiro */
.file-tag {
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
/* Efeito na área de ficheiros ao passar o rato */
...
/* Exibição do progresso de rondas no rodapé */
...
/* Cores do estado de progresso - rodapé */
...
/* ===== Estilos do modal de detalhes do histórico ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
...
/* Estilos personalizados da barra de rolagem */
