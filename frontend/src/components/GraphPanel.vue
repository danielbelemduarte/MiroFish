  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="selectedItem" class="detail-panel">
          ...
          <div v-else class="detail-content">
            <!-- Detalhes do grupo de auto-referência -->
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>A carregar dados do grafo...</p>
      </div>
      
      <!-- Estado de espera/vazio -->
      <div v-else class="graph-state">
        <div class="empty-icon">❖</div>
        <p class="empty-text">A aguardar geração da ontologia...</p>
      </div>
    </div>
    ...
    <!-- Interruptor de exibição de etiquetas das arestas -->
...
const showEdgeLabels = ref(true) // Mostrar etiquetas das arestas por defeito
const expandedSelfLoops = ref(new Set()) // Itens de auto-referência expandidos
const showSimulationFinishedHint = ref(false) // Dica após o fim da simulação
const wasSimulating = ref(false) // Rastrear se estava em simulação anteriormente

// Fechar dica de fim de simulação
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Monitorizar alterações em isSimulating, detetar fim da simulação
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitou de estado de simulação para não-simulação, mostrar dica de conclusão
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Alternar estado expandido/recolhido dos itens de auto-referência
...
const entityTypes = computed(() => {
  ...
  const typeMap = {}
  // Paleta de cores esteticamente agradável
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Repor estado de expansão
}
...
const renderGraph = () => {
  ...
  const edgePairCount = {}
  const selfLoopEdges = {} // Arestas de auto-referência agrupadas por nó
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Contar número de arestas entre cada par de nós, recolher arestas de auto-referência
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Auto-referência - recolher para array
  ...
  // Registar o índice atual da aresta processada para cada par de nós
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Nós de auto-referência já processados
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Aresta de auto-referência - adicionar apenas uma auto-referência combinada por nó
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Armazenar informações detalhadas de todas as arestas de auto-referência
        }
      })
      return
    }
    ...
    // Verificar se a direção da aresta coincide com a direção normalizada (UUID origem < UUID destino)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calcular curvatura: distribuir quando há múltiplas arestas, linha reta para aresta única
    let curvature = 0
    if (totalCount > 1) {
      // Distribuir curvatura uniformemente, garantir distinção clara
      // O intervalo de curvatura aumenta com o número de arestas
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Se a direção da aresta for oposta à normalizada, inverter curvatura
      // Isto garante que todas as arestas se distribuem no mesmo referencial, sem sobreposição por diferença de direção
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulação - ajustar dinamicamente o espaçamento dos nós com base na quantidade de arestas
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Ajustar distância dinamicamente com base na quantidade de arestas entre este par de nós
      // Distância base 150, acrescentar 40 por cada aresta adicional
    ...
    // Adicionar gravidade central para agrupar nós independentes na zona central
  ...
  const getLinkPath = (d) => {
    ...
    // Detetar auto-referência
    if (d.isSelfLoop) {
      // Auto-referência: desenhar um arco a partir do nó e de volta
      const loopRadius = 30
      // Partir do lado direito do nó, dar a volta e regressar
      const x1 = sx + 8  // Desvio do ponto inicial
      const y1 = sy - 4
      const x2 = sx + 8  // Desvio do ponto final
      const y2 = sy + 4
      // Usar arco para desenhar auto-referência (sweep-flag=1 sentido horário)
    ...
    // Calcular ponto de controlo da curva - ajustar dinamicamente com base na quantidade de arestas e distância
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Desvio perpendicular à direção da ligação, calculado proporcionalmente à distância, garantindo que a curva seja visível
    // Quanto mais arestas, maior a proporção de desvio em relação à distância
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, acrescentar 5% por cada aresta adicional
  ...
  // Calcular ponto médio da curva (para posicionamento da etiqueta)
  const getLinkMidpoint = (d) => {
    ...
    // Detetar auto-referência
    if (d.isSelfLoop) {
      // Posição da etiqueta de auto-referência: lado direito do nó
      return { x: sx + 70, y: sy }
    }
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Repor o estilo da aresta anteriormente selecionada
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // Destacar a aresta atualmente selecionada
  ...
  // Guardar referência para controlo externo de visibilidade
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Apenas registar posição, não reiniciar simulação (distinguir clique de arrastar)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detetar se realmente começou a arrastar (movimento acima do limiar)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Primeira deteção de arrastar real, só então reiniciar simulação
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Só deixar a simulação parar gradualmente se realmente houve arrasto
        if (d._isDragging) {
    ...
    .on('click', (event, d) => {
      event.stopPropagation()
      // Repor o estilo de todos os nós
      node.attr('stroke', '#fff').attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      // Destacar nó selecionado
      d3.select(event.target).attr('stroke', '#E91E63').attr('stroke-width', 4)
      // Destacar arestas ligadas a este nó
  ...
  simulation.on('tick', () => {
    ...
    // Atualizar posição das etiquetas das arestas (sem rotação, exibição horizontal mais legível)
    ...
    // Atualizar fundo das etiquetas das arestas
  ...
  // Clicar na área vazia para fechar o painel de detalhes
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Monitorizar o interruptor de exibição das etiquetas das arestas
...
/* Estilo da dica após o fim da simulação */
