...
Optimizations:
1. Leverage graph search to further enrich node information
...
from ..utils.llm_client import create_openai_client, get_model_name, _is_reasoning_model
...
class OasisProfileGenerator:
    ...
    Converts graph entities into Agent Profiles required by OASIS simulation.

    Optimized features:
    1. Leverages graph search to obtain richer context
    ...
    def __init__(
        ...
        # Graph client for enriching context
        self.graph_client = LocalGraphClient()
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use graph hybrid search to obtain rich information about an entity.

        The graph does not have a built-in hybrid search endpoint, so edges and nodes
        ...
        try:
            ...
            logger.info(f"Graph search done: {entity_name}, {len(results['facts'])} facts, {len(results['node_summaries'])} nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 4. Use graph hybrid search to obtain richer information
        ...
        if zep_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Retrieved via Search\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
        
        if zep_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved via Search\n" + "\n".join(f"- {s}" for s in zep_results["node_summaries"][:10]))
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if not _is_reasoning_model(self.model_name):
                    create_kwargs["temperature"] = 0.7 - (attempt * 0.1)
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally reflecting real-world conditions. You must return valid JSON format, and all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximally reflecting real-world conditions.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Personal memories (important part of the persona: introduce this individual's connection to events, and their existing actions and reactions in those events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, maximally reflecting real-world conditions.
...
- Institutional account communications must align with its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID for graph search to obtain richer context
            parallel_count: Number of parallel generations, default 5
            realtime_output_path: File path for real-time writing (writes after each generation if provided)
