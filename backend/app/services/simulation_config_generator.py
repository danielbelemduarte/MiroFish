...
- The user group is Chinese, so the schedule should follow Beijing time daily patterns
- 0-5 AM: almost no activity (activity multiplier 0.05)
- 6-8 AM: gradually becoming active (activity multiplier 0.4)
- 9 AM-6 PM: moderately active during work hours (activity multiplier 0.7)
- 7-10 PM: peak period (activity multiplier 1.5)
...
- **Important**: The example values below are for reference only. You should adjust specific time periods based on the nature of the event and characteristics of participant groups.
  ...
  - Example: Breaking news may lead to late-night discussions, so off_peak_hours can be shortened accordingly
...
For example: Official statements should be posted by Official/University types, news by MediaOutlet, student perspectives by Student.
...
- **Schedule follows Chinese daily patterns**: Almost no activity from 0-5 AM, most active during 7-10 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations should follow Chinese daily activity patterns."
