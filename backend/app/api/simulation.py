...
def prepare_simulation():
    ...
    Features:
    - Auto-detects completed preparation work to avoid duplicates
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task doesn't exist, but if there's a simulation_id, check if preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (LLM-generated complete config).
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) are updated to the graph in real-time
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get actions for current round (recent_actions shows only latest round)
        ...
        # recent_actions shows only current latest round for both platforms
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been shut down. Please ensure the simulation has completed and is in command-wait mode."
            }), 400
...
def close_simulation_env():
    ...
    Send shutdown command to simulation, allowing it to gracefully exit command-wait mode.

    Note: This differs from the /stop endpoint. /stop forcefully terminates the process,
    while this endpoint lets the simulation gracefully shut down and exit.
    ...
    try:
        ...
        logger.error(f"Failed to shutdown environment: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
